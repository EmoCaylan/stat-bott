...
logs ( client );
let pub = "835544861033365521" ////// kategori id
let kayıt = "835544857916473347"
let terapi = "835544859976663055"
let sorun = "835544860357689349"
let vk = "835544863242977288"
let dc = "835544863242977282"
let priv = "835544860706603063"
let alone = "835544862198726666"
let erkek = "835544857594429451"
let kız = "835544857594429454"

client.on("ready", async () => {
  client.user.setPresence({ activity: { name: "✩Sword❤️Emirhan"}, status: "online" }); //dnd rahatsı etmeyın, online çevirimiçi, idle boşta 
});
...
client.on ( "voiceChannelJoin" , ( member , channel ) => {
    ...
    if (!member.roles.cache.has(erkek) && !member.roles.cache.has(kız)) return
    const json = {
        "channel" : channel.id ,
...
client.on ( "message" , async message => {
    ...
    if (!message.member.roles.cache.has(erkek) && !member.roles.cache.has(kız)) return
    var totall = (
                     await db.fetch (
...
client.on ( "message" , async msg => {
    if ( msg.content.startsWith ( ".stat" ) || msg.content.startsWith ( ".me" ) ) {
        ...
        const embed = new Discord.MessageEmbed ()
            ...
            .setColor ( üye.displayHexColor )
            .setDescription ( `${ üye } (${üye.roles.highest}) Kişisinin Sunucudaki istatistikleri
───────────────
...
• Stream Odalar: \`${ toplamPriv }\`
• Secret Odalar: \`${ toplamAlone }\`
...
client.on ( "message" , async msj => {
    ...
    const toplam = new Discord.MessageEmbed ()
        .setAuthor ( msj.guild.name , msj.guild.iconURL({dynamic: true}) )
...
client.on("ready", async () => { 
let botVoiceChannel = client.channels.cache.get("835544858604470336"); 
if (botVoiceChannel) 
